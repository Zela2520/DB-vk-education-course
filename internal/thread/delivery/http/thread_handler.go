...
import (
	...
	"TechnoParkDBProject/internal/middleware"
	"TechnoParkDBProject/internal/pkg/responses"
	"TechnoParkDBProject/internal/thread"
	"TechnoParkDBProject/internal/thread/models"
	"encoding/json"
	"fmt"
...
func NewThreadHandler(router *router.Router, threadUsecase thread.Usecase,
	...
	router.POST("/api/forum/{slug}/create",
		middleware.ContentTypeJson(threadHandler.CreateThread))
	router.GET("/api/forum/{slug}/threads", middleware.ContentTypeJson(threadHandler.GetThreads))
	router.GET("/api/thread/{slug_or_id}/details", middleware.ContentTypeJson(threadHandler.GetThread))
	router.POST("/api/thread/{slug_or_id}/details", middleware.ContentTypeJson(threadHandler.UpdateThread))
