...
import (
	...
	"TechnoParkDBProject/internal/user/models"
	"encoding/json"
	"fmt"
...
func NewUserHandler(router *router.Router, userUsecase user.Usecase) *UserHandler {
	...
	userHandler.router.POST("/api/user/{nickname}/create",
		middleware.ContentTypeJson(userHandler.CreateUserHandler))
	userHandler.router.GET("/api/user/{nickname}/profile",
		middleware.ContentTypeJson(userHandler.GetUserHandler))
	userHandler.router.POST("/api/user/{nickname}/profile",
		middleware.ContentTypeJson(userHandler.UpdateUserHandler))
	userHandler.router.POST("/api/service/clear",
		middleware.ContentTypeJson(userHandler.DeleteAllHandler))
	userHandler.router.GET("/api/service/status",
		middleware.ContentTypeJson(userHandler.GetStatus))
	return userHandler
}
